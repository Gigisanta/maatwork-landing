 ...
 * cycles the 4 real ecosystem products. Mission Control framing (mono status
 * header + KPI cells + signal rows), now alive: per-product accent color, a
 * drifting scan beam, animated KPI ticks and pulsing live status. Data is DEMO
 * (labelled), modelled on each app's real entities — no invented metrics.
 * Pauses on hover/offscreen, tabs clickable, static with reduced motion.
 */
import { useEffect, useRef, useState, type CSSProperties } from "react";

type Severity = "ok" | "info" | "warning" | "critical";
type Accent = "cyan" | "violet" | "gold" | "rose";
...
type Screen = {
  ...
  role: string;
  accent: Accent;
  kpis: { label: string; value: string; delta?: string; fill: number }[];
...
const SCREENS: Screen[] = [
  ...
  },
  {
    ...
    accent: "violet",
    kpis: [
      { label: "Deals abiertos", value: "24", delta: "+3 mes", fill: 0.66 },
      { label: "Tareas hoy", value: "8", fill: 0.4 },
      { label: "Contactos", value: "1.4k", fill: 0.82 },
    ],
    rows: [
  ...
  },
  {
    ...
    accent: "gold",
    kpis: [
      { label: "Obras activas", value: "12", delta: "+2 mes", fill: 0.6 },
      { label: "Avance prom.", value: "68%", delta: "+4 pp", fill: 0.68 },
      { label: "Alertas", value: "3", delta: "2 críticas", fill: 0.3 },
    ],
    rows: [
  ...
  },
  {
    ...
    accent: "rose",
    kpis: [
      { label: "Extintores", value: "486", fill: 0.9 },
      { label: "Órdenes activas", value: "17", fill: 0.55 },
      { label: "Vencen (mes)", value: "29", delta: "agenda llena", fill: 0.62 },
    ],
    rows: [
...
export function ProductShowcase() {
  ...
  return (
    <div
      ref={rootRef}
      className={`product-frame accent-${accent} relative mx-auto max-w-5xl overflow-hidden rounded-[20px] border p-2 transition-colors duration-500`}
    ...
    >
      ...
      {/* stacked product screens (crossfade) + live scan beam */}
      <div className="relative h-[300px] w-full overflow-hidden bg-bg-base sm:h-[348px]">
        ...
        {SCREENS.map((s, idx) => (
          ...
          >
            <Panel screen={s} live={idx === active} />
          </div>
        ))}
      </div>

      {/* product tabs — colored by active accent */}
        ...
        {SCREENS.map((s, idx) => {
          ...
          return (
            <button
              ...
              aria-pressed={on}
              className={`accent-${s.accent} relative flex items-center justify-center gap-1.5 overflow-hidden rounded-lg border px-2 py-2 font-mono text-[10.5px] uppercase tracking-[0.08em] transition-colors ${
                on
                  ? "text-white"
                  : "border-white/[0.05] bg-white/[0.015] text-slate-500 hover:text-slate-300"
              }`}
              style={on ? { borderColor: "var(--accent-ring)", background: "var(--accent-soft)", color: "#fff" } : undefined}
            >
              <span
                className="h-1.5 w-1.5 shrink-0 rounded-full"
                style={{ background: on ? "var(--accent)" : "rgba(255,255,255,0.18)" }}
                aria-hidden
              />
              ...
              {on && !paused && visible && pageVisible && (
                <span
                  ...
                  style={{ animationDuration: `${INTERVAL_MS}ms`, background: "linear-gradient(90deg, var(--accent-2), var(--accent))" }}
                />
              )}
            </button>
          );
...
function Panel({ screen, live }: { screen: Screen; live: boolean }) {
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="mt-3 grid grid-cols-3 gap-2">
        {screen.kpis.map((k) => (
          <div key={k.label} className="rounded-lg border border-white/[0.07] bg-white/[0.02] px-2.5 py-2">
            ...
            {/* animated accent tick — decorative proportion, not a claimed metric */}
            <div className="kpi-bar mt-1.5" aria-hidden>
              {live && <span style={{ "--fill": k.fill } as CSSProperties} />}
            </div>
            {k.delta && <p className="mt-1 truncate font-mono text-[9px] text-success/85 sm:text-[10px]">{k.delta}</p>}
          </div>
        ))}
      </div>
      ...
      <div className="mt-2.5 flex-1 space-y-1.5 overflow-hidden">
        {screen.rows.map((r, i) => (
          <div
            key={r.name}
            className="flex items-center gap-2.5 rounded-md border border-white/[0.05] bg-white/[0.015] px-2.5 py-1.5 transition-colors sm:py-2"
            style={{ borderLeft: "2px solid var(--accent-ring)" }}
          >
            <span className={`signal-dot ${DOT[r.sev]} ${live && i === 0 ? "dot-pulse" : ""}`} aria-hidden />
