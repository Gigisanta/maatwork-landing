...
/**
 * Safari — port del componente de MagicUI (magicui.design/docs/components/safari):
 * marco de ventana Safari dibujado como SVG que se superpone (con una máscara que
 * "recorta" la pantalla) sobre el contenido. A diferencia del original, el chrome
 * está tematizado en oscuro fijo para calzar con el DS (el sitio no togglea `dark`),
 * y se agrega soporte de `children`: cualquier UISe renderiza dentro de la pantalla
 * y se auto-escala con un ResizeObserver, quedando nítida como un screenshot real
 * a cualquier ancho (móvil incluido).
 */
...
// Chrome oscuro (determinista, no depende de prefers-color-scheme)
const CHROME = "#2A2A31"; // barra/cuerpo del marco
const CHROME_FACE = "#17171B"; // cara de la toolbar
const ICON = "#8A8A94";
...
export function Safari({
  ...
  // Auto-escalado: la UI se diseña a SCREEN_WIDTH×SCREEN_HEIGHT y se escala al
  // ancho real de la pantalla — el aspect del slot coincide 1:1 con el diseño.
  ...
  useEffect(() => {
    ...
    if (!el) return;
    // Recalcula el factor de escala contra el ancho real de la pantalla.
    // RO cubre cambios del elemento; el listener de resize + un rAF inicial
    // cubren contextos embebidos donde RO no dispara fiable (mismo motivo por
    // el que Terminal tiene fallbacks).
    const update = () => {
      const w = el.clientWidth;
      if (w > 0) setScale(w / SCREEN_WIDTH);
    };
    update();
  ...
  return (
    <div
      className={`relative block align-middle leading-none ${className ?? ""}`}
      style={{ width: "100%", aspectRatio: `${SAFARI_WIDTH}/${SAFARI_HEIGHT}`, ...style }}
      {...props}
    >
      {hasVideo && (
        <div
      ...
      >
        ...
        <g clipPath="url(#path0)" mask={hasMedia ? "url(#safariPunch)" : undefined}>
          ...
          <path
            ...
            fill={CHROME_FACE}
          />
          <circle cx="27" cy="25" r="6" fill={CHROME} />
          <circle cx="47" cy="25" r="6" fill={CHROME} />
          <circle cx="67" cy="25" r="6" fill={CHROME} />
          <path
          ...
          <g className="mix-blend-luminosity">
            <text x="580" y="30" fill={ICON} fontSize="12" fontFamily="Arial, sans-serif">
              {url}
            </text>
          </g>
